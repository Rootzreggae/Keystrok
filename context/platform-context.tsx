...
type PlatformContextType = {
  ...
  isLoading: boolean
  error: string | null
  getPlatformStatus: () => { hasPlatforms: boolean }
}
...
export function PlatformProvider({ children }: { children: React.ReactNode }) {
  ...
  // Save platforms to localStorage as a backup when they change
  useEffect(() => {
    if (initialized) {
      try {
        localStorage.setItem("keystrok-platforms", JSON.stringify(platforms))
      } catch (error) {
        console.error("Error saving platforms to localStorage:", error)
      }
    }
  }, [platforms, initialized])

  // Add this function to the PlatformProvider component
